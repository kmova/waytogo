...
// CmdDaemon execs waytogod with the same flags
func (p DaemonProxy) CmdDaemon(args ...string) error {
	// Special case for handling `waytogo help daemon`. When pkg/mflag is removed
	...
	if len(args) == 0 || args[0] != "--help" {
		// Use os.Args[1:] so that "global" args are passed to waytogod
...
// findDaemonBinary looks for the path to the waytogod binary starting with
