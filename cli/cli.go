...
func New(handlers ...Handler) *Cli {
	...
	// in order to handle `waytogo help` appropriately
	cli := new(Cli)
...
func (cli *Cli) noSuchCommand(command string) {
	...
	fmt.Fprintf(cli.Stderr, "waytogo: '%s' is not a waytogo command.\nSee 'waytogo --help'.\n", command)
	os.Exit(1)
}
...
// CmdHelp displays information on a WayToGo command.
//
// If more than one command is specified, information is only shown for the first command.
//
// Usage: waytogo help COMMAND or waytogo COMMAND --help
...
// Subcmd is a subcommand of the main "waytogo" command.
// A subcommand represents an action that can be performed
// from the WayToGo command line client.
//
// To see all available subcommands, run "waytogo --help".
func Subcmd(name string, synopses []string, description string, exitOnError bool) *flag.FlagSet {
	...
	flags.ShortUsage = func() {
		...
		for i, synopsis := range synopses {
			...
			fmt.Fprintf(flags.Out(), "\n%swaytogo %s%s", lead, name, synopsis)
		}
