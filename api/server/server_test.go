...
import (
	...
	"github.com/kmova/waytogo/api/server/httputils"
...
func TestMiddlewares(t *testing.T) {
	...
	localHandler := func(ctx context.Context, w http.ResponseWriter, r *http.Request, vars map[string]string) error {
		...
		if sv := w.Header().Get("Server"); !strings.Contains(sv, "WayToGo/0.1omega2") {
			t.Fatalf("Expected server version in the header `WayToGo/0.1omega2`, got %s", sv)
		}
