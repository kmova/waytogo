...
func checkKernel() error {
	...
	if !checkKernelVersion(3, 10, 0) {
		...
		if os.Getenv("WAYTOGO_NOWARN_KERNEL_VERSION") == "" {
			logrus.Fatalf("Your Linux kernel version %s is not supported for running WayToGo. Please upgrade your kernel to 3.10.0 or newer.", v.String())
		}
	}
	return nil
}
...
func checkSystem() error {
	if os.Geteuid() != 0 {
		return fmt.Errorf("The WayToGo daemon needs to be run as root")
	}
	return checkKernel()
}
