...
import (
	...
	"github.com/kmova/waytogo/pkg/mflag"
)

func TestDaemonConfigurationMerge(t *testing.T) {
	f, err := ioutil.TempFile("", "waytogo-config-")
	if err != nil {
		t.Fatal(err)
	}
...
func TestDaemonConfigurationNotFound(t *testing.T) {
	_, err := MergeDaemonConfigurations(&Config{}, nil, "/tmp/foo-bar-baz-waytogo")
...
func TestFindConfigurationConflictsWithNamedOptions(t *testing.T) {
	...
	if err := flags.Set("H", "unix:///var/run/waytogo.sock"); err != nil {
		t.Fatal(err)
	}
...
func TestDaemonConfigurationMergeConflictsWithInnerStructs(t *testing.T) {
	...
	flags.Set("tlscacert", "~/.waytogo/ca.pem")
...
func TestFindConfigurationConflictsWithMergedValues(t *testing.T) {
	...
	flags.Set("-host", "unix:///var/run/waytogo.sock")
	err = findConfigurationConflicts(config, flags)
	if err == nil {
		t.Fatal("expected error, got nil")
	}
	if !strings.Contains(err.Error(), "hosts: (from flag: [unix:///var/run/waytogo.sock], from file: tcp://127.0.0.1:2345)") {
		t.Fatalf("expected hosts conflict, got %v", err)
	}
}
