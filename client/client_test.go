...
func TestNewEnvClient(t *testing.T) {
	cases := []struct {
		...
		{
			envs: map[string]string{
				"WAYTOGO_CERT_PATH": "invalid/path",
			},
		...
		},
		{
			...
			expectedError: "unable to parse waytogo host `host`",
		},
		{
			envs: map[string]string{
				"WAYTOGO_HOST": "invalid://url",
			},
			expectedVersion: DefaultVersion,
		},
		{
			envs: map[string]string{
				"WAYTOGO_API_VERSION": "anything",
			},
			expectedVersion: "anything",
		},
		{
			envs: map[string]string{
				"WAYTOGO_API_VERSION": "1.22",
			},
...
func TestGetAPIPath(t *testing.T) {
	...
	for _, cs := range cases {
		c, err := NewClient("unix:///var/run/waytogo.sock", cs.v, nil, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestNewEnvClientSetsDefaultVersion(t *testing.T) {
	...
	expected := "1.22"
	os.Setenv("WAYTOGO_API_VERSION", expected)
	client, err = NewEnvClient()
	if err != nil {
		t.Fatal(err)
	}
